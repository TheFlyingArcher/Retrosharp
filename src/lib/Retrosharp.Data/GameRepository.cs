...
namespace Retrosharp.Data
{
    public class GameRepository : BaseRepository<GameModel, Game>, IGameRepository
    {
        ...
        public async Task<IEnumerable<Game>> GetByFranchiseSeasonAsync(int franchiseId, short seasonYear)
        {
            var games = await Context.Set<GameModel>()
        ...
        public async Task<IEnumerable<Game>> GetBySeasonAsync(short seasonYear)
        {
            var games = await Context.Set<GameModel>()
                .Where(g => g.GameDate.Year == seasonYear)
                .ProjectToType<Game>()
                .ToListAsync();
