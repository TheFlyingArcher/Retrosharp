...
### `Batting` was missing RBI and Games Played/Started

Step 6d's original derivation left `Batting` with no `RunsBattedIn`, `GamesPlayed`, or `GamesStarted` — RBI existed only at the play level (`GameEventRunner.IsRBI`) and the team level (`GameBattingStatistics.RunsBattedIn`, already reconciled independently by `GameReconciliationResolver`), and batter G/GS didn't exist anywhere, unlike `Pitching`'s equivalents. See [frontend-prototype.md](./frontend-prototype.md#resolved-rbi-and-games-playedstarted-were-missing-from-batting) for the full fix and its scope: RBI and GamesPlayed are self-contained per game and flow through every derivation path (season aggregate, per-game log, box score); GamesStarted is only populated on the season-aggregate path (`GameEventImportService`, which has access to the event file's "start" records) and reads as `0` on the on-demand per-game paths (`GET /players/{personId}/games`, `GET /games/{gameId}`'s box score), which don't have starting-lineup data available to them.
...
- **Line score**: already stored (`Game.VisitorLineScore`/`HomeLineScore`), just never mapped onto `GameSummaryResponse` — a response-shape fix, not a new derivation. Implemented as a `LineScore` field on `GameTeamBoxScore`/`GameTeamBoxScoreResponse` (alongside that type's existing per-team `Runs`/`Hits`/`Errors`), not as two top-level `VisitorLineScore`/`HomeLineScore` fields.
- **Starting pitchers**: Retrosheet's Game Log format records each team's starting pitcher explicitly (fields 102-105, distinct from the batting lineup, since a DH-era starting pitcher never bats). The Game Log Parser already reads `VisitorStartingPitcherId`/`HomeStartingPitcherId` into its raw import object (`GameLog.cs`), but never persists them — `Game`/`GameModel` has no corresponding column. Deriving the starter from the batting lineup's "P" position instead doesn't work, since a DH-era starter is never in that lineup at all. Implemented as top-level `VisitorStartingPitcher`/`HomeStartingPitcher` on `GameSummaryResponse`, matching `WinningPitcher`/`LosingPitcher`/etc.'s existing shape.
- **Backfilling already-imported games is not automatic**: `GameRepository.BulkInsertAsync` is skip-only, not upsert, so re-running the Game Log Parser over an already-imported season will not populate these new columns on existing rows — it needs a wipe-and-reimport (the pattern Step 7f's `RecordIndex` backfill used) or a one-off data-fix script. See [phase-1-build-plan.md Step 7h](./phase-1-build-plan.md#step-7h-game-summary-enrichment-starting-pitchers-and-line-score) for the corrected note (an earlier draft of this step incorrectly assumed re-import alone was sufficient).
...
**Implementation note**: this ended up needing no new grouping query at all. Step 6d's `GameStatisticsResolver.Resolve(...)` — already used by `PlayerGameLogService` to derive one player's per-game delta — computes deltas for **every** batter and pitcher in a game in one pass (`GameStatisticsDelta.Battings`/`Pitchings`); `PlayerGameLogService` was just filtering that full list down to one `PersonId`. Removing the filter and splitting by `FranchiseId` instead is the entire "new" derivation. Defensive position was the one genuinely new piece: resolved by combining `GameLineup` (starting position) with `GameSubstitution` (Retrosheet records a player changing position while remaining in the game as another substitution row for the same person, not just entries/exits), via a pure, unit-tested `GameSummaryService.ResolvePosition` — see [phase-1-build-plan.md Step 7i](./phase-1-build-plan.md#step-7i-full-game-box-score-all-participants) for the full implementation notes. Response shape ended up as `GameBoxScoreBattingParticipant`/`GameBoxScorePitchingParticipant` (new types, `Player` + `Position` + nested stats) rather than literally reusing `GameBattingLine`/`GamePitchingLine`, since those carry `GameId`/`GameDate`/`FranchiseCode` fields that are redundant once already nested under one game's one team's box score.
...
**Implementation note**: the new table is `GameEventContext` (`GameId` primary key/foreign key, `StartTimeLocal` nullable `TimeOnly`) — deliberately not reusing `GameEventGameStatus` (a narrow concurrency-control marker), matching the constraint already called out above. `EventFileReader`'s `starttime` parsing is best-effort (`TimeOnly.TryParseExact` against Retrosheet's `h:mmtt` format, e.g. `"7:44PM"`): a missing or malformed value leaves `StartTime` null rather than failing the whole game's import, unlike the four `info` fields (`hometeam`/`visteam`/`date`/`number`) that are genuinely required to build an `EventFileGame` at all. See [phase-1-build-plan.md Step 7j](./phase-1-build-plan.md#step-7j-game-start-time-from-event-file-info-records) for the full implementation notes, including one unverified assumption worth flagging: Retrosheet's exact tolerance for malformed `starttime` values (extra whitespace, a 24-hour clock variant, etc.) wasn't checked against real-world format variance beyond the fixture data available in this session.
...
### Standings are precomputed, not live-queried

No part of the codebase computed a franchise's win-loss record — `TeamStatisticsService` only ever aggregated batting/pitching/fielding counting stats. This blocked the Wins/Losses/Win %/Finish/Divisions/Pennants/GB columns on the Franchise Detail, Franchises, and Season Detail pages (see [frontend-prototype.md](./frontend-prototype.md#resolved-standings-derivation) for the full writeup).

Fixed with a new `FranchiseSeasonStanding` table, deliberately **precomputed per season** rather than derived live on each request — unlike a rate stat like AVG (self-contained per row), a franchise's rank and games-behind are only meaningful relative to every other franchise in the same grouping for that season, so computing them live on every `GET /teams/{id}/stats` call would mean re-deriving the whole season's standings on every single-franchise request. A new pure `StandingsResolver` (mirroring `GameStatisticsResolver`'s style) tallies wins/losses/ties from that season's `Game` rows, groups franchises by division when their era's `Franchise.DivisionCode` is populated (otherwise by league alone), and ranks within each group.

Two grouping passes run independently: a league-wide pass (every franchise-season, regardless of division) determines `LeagueBestRecord`; a division-scoped pass (only franchise-seasons with a division) determines `DivisionChampion` and overrides `Rank`/`GamesBehind` for those franchise-seasons. A franchise-season with no division keeps the league-wide `Rank`/`GamesBehind`, matching this page's own before/after-1969 rule for GB and Finish. `LeagueBestRecord` is the pre-1969 pennant exactly as this project defines it, but is **not** a valid stand-in for a post-1969 pennant (League Championship Series winner) — postseason results aren't imported by any parser. This is the same scope boundary already established for the Seasons page's League Champion column.

Recomputation is triggered by `POST /standings/compute?season=` (not listed in the read-only [API Surface](#api-surface) table below, since — like the Person/GameLog/GameEvent import-trigger endpoints — it's a write, not a Data Viewing read). Unlike those import endpoints, it doesn't run as an NServiceBus saga: it has no external file to parse, no multi-stage retry-worthy failure mode, and completes synchronously as a single in-memory aggregation over already-imported `Game` rows for one season (at most a few thousand rows) — full saga/service-bus ceremony would be disproportionate. It's fully idempotent: `ReplaceSeasonAsync` atomically deletes and reinserts that season's rows, so re-running it after importing more of that season's Game Log data simply recomputes from whatever games exist now.

### Franchise all-time summaries are computed from every era in a lineage, not just one

The Franchises page (see [frontend-prototype.md](./frontend-prototype.md#resolved-franchise-all-time-summary)) needs all-time Wins/Losses/Win %/Above-.500/Below-.500, one row per franchise *lineage* displayed under its current name (Washington Nationals, not Montreal Expos) — distinct from a single `Franchise` row, since a renamed/relocated franchise is multiple `Franchise` eras sharing one `FranchiseIdentifier`.

Fixed with a new pure `FranchiseCareerSummaryResolver`: groups `Franchise` rows by `FranchiseIdentifier`, picks the most recent era (by `FranchiseStart`) to represent the lineage's current name and franchise code, collects every earlier era's name as a `FormerNames` entry (oldest first), and sums `FranchiseSeasonStanding` rows across **every** era's `FranchiseId` in the lineage — a franchise's pre-rename seasons still count toward its all-time record. `SeasonsAboveFiveHundred`/`SeasonsBelowFiveHundred` count per-season win percentage strictly above/below .500 across every era; a season at exactly .500 counts toward neither, matching the same convention `StandingsResolver`'s ranking tie-break already established for win percentage.
...
| `GET /teams?limit=&offset=` | Browse one all-time career summary per franchise lineage (every era collapsed into one row, current name plus former names), ordered by current name. Backs the Franchises page, distinct from the search below (see [Considerations](#franchise-all-time-summaries-are-computed-from-every-era-in-a-lineage-not-just-one)). |
