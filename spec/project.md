...
1. Postseason data import: parse and import Retrosheet's postseason game logs and event files (World Series, League Championship Series, Division Series, Wild Card games), extending the same Game Log Parser/Game Event Parser pipeline to a second, postseason-flagged set of files. This unblocks a true single per-season champion, a correct post-1969 "Pennant" (League Championship Series winner, distinct from the pre-1969 best-regular-season-record proxy Phase 1 uses instead), and postseason entries for the home page's history widget. Noticed three separate times during Phase 1's frontend prototyping pass, each independently scoped down or worked around rather than built: the Seasons page's [League Champion](./frontend-prototype.md#resolved-league-champion-is-out-of-scope-as-originally-worded) column, the Franchise Detail page's post-1969 [Pennants](./frontend-prototype.md#resolved-standings-derivation) column, and the Home Page's ["On This Day in Baseball History"](./frontend-prototype.md#resolved-on-this-day-in-baseball-history-widget) widget, which can't surface postseason moments (a Series-winning home run, a walk-off in Game 7) without this data. Deliberately kept out of Phase 1 scope each time.
1. Team logos: display each franchise's logo as it appeared during a given era (e.g. on the game box score and franchise pages), sourced from a hand-maintained image asset library keyed by franchise era — Retrosheet has no branding assets, so this can never be derived from imported data the way everything else in this project is. Noticed during Phase 1's frontend prototyping pass (game box score design) but deliberately kept out of Phase 1 scope; omitted entirely from the Phase 1 prototype.
