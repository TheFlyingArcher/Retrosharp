...
### Step 7e: Team Search, Roster, and Season Statistics

**Status**: Complete

**What was found (starting state)**: Nothing pre-existing to correct — this is all new surface. Researching it did surface a genuine reuse opportunity, though: HR/9, HR/FB, FIP, and pitcher BABIP for a **team** need almost exactly the same machinery Step 7c already built for a **player** — `PitcherEventAggregateResolver.Resolve()` already groups its input by `(FranchiseId, SeasonYear)`, so feeding it every event for one franchise-season (instead of one pitcher's events) produces the team-wide aggregate for free, no new resolver code needed. Given that reuse, team pitching got the full stat set rather than being scoped down to just the stats directly summable from `Pitching` rows.

**What was built**:
- `TeamBattingStatistics : GameBattingStatistics` / `TeamFieldingStatistics : GameFieldingStatistics` (`Retrosharp.Contract.Game`): the same rate-stat formulas as `BattingStatistics`/`FieldingStatistics`, referencing `GameBattingStatistics.Hit` (correctly named from the start, unlike `Batting.Hits`'s own history). Team pitching reuses the **existing** `PitchingStatistics` class as-is.
- `IFipConstantResolver`/`FipConstantResolver` (`Retrosharp.Service`): extracted from what was a private method on `PlayerStatisticsService`, so player and team stats share one FIP-constant implementation instead of two copies. `PlayerStatisticsService` updated to call it instead of its own inlined logic.
- New repository methods, each following the exact pattern already established: `IFranchiseRepository.SearchAsync` (free-text over `Nickname`/`PlayingCity`/`AlternateNickname`, exact-match `FranchiseCode`, optional season-era disambiguation — the same date-range logic `GetByFranchiseCodeAndDateAsync` already uses); `IBattingRepository`/`IPitchingRepository`/`IFieldingRepository.GetByFranchiseAsync`; `IGameBattingStatisticsRepository`/`IGameFieldingStatisticsRepository.GetByFranchiseSeasonAsync` (joined through `Game` for the season filter); `IGameEventRepository.GetTeamPitchingEventsAsync` (the same shape as 7c's `GetPitcherGameEventsAsync`, filtered by fielding franchise instead of `PitcherId`).
- New `ITeamService`/`TeamService` (search/detail/roster — roster resolves distinct `PersonId`s across `Batting`/`Pitching`/`Fielding` for the franchise(-season), then resolves each to identity) and `ITeamStatisticsService`/`TeamStatisticsService` (season batting/pitching/fielding, with `GamePitchingStatistics.TeamEarnedRuns` substituted in for the pitching ERA numerator, per `api.md`'s resolution).
- New `TeamsController`: `GET /api/teams/search|{id}|{id}/roster|{id}/stats`. Roster reuses 7a's `PlayerSearchResult` DTO directly; season stats reuses 7b/7c's `BattingLine`/`PitchingLine`/`FieldingLine` DTOs directly — a team-season line is the same shape as a player's, just without the null-when-combined fields.

**Discrepancies and decisions made during implementation**:
- Unlike player search, neither `q` nor `code` is required for team search — franchises are a small, bounded reference set (125 rows total, confirmed in Step 2), so browsing all teams paginated is a legitimate request, not the unbounded scan player search was guarding against.
- `season` is required on `GET /teams/{id}/stats` (`400` if missing) — unlike a player's career stats, a team's stats without a season isn't a standard, commonly-needed concept, so it wasn't built.
- **A real bug found live, from reusing the `BattingLine` DTO across two different source shapes**: `TeamBattingStatistics.Hit` (singular, inherited from `GameBattingStatistics`, correctly named there) doesn't name-match `BattingLine.Hits` (plural, the DTO's field name from when it was built for player-level `Batting.Hits` in 7b) — Mapster's `Adapt<BattingLine>()` silently left `Hits` at `0`, exactly the same class of bug as 7b's original `Batting.Hit`/`Hits` finding, just surfacing again at the DTO-mapping layer this time instead of the EF-model layer. Caught immediately because `BattingAverage`/`OnBasePercentage`/`SluggingPercentage`/`TotalBases` were all clearly non-zero and sensible while `hits` alone read `0` — the computed properties reference `Hit` directly within `TeamBattingStatistics` itself, bypassing the broken mapping entirely, so the inconsistency was self-evident. Fixed with an explicit post-map assignment in the controller (`battingLine.Hits = battingStats.Hit`), the same style of fixup already used elsewhere (`PitchingLine.InningsPitchedDisplay` in 7d).

**Errors encountered**: The `Hit`/`Hits` DTO-mapping bug above — found and fixed within this step, not a regression of the already-fixed 7b column-naming issue (that fix was correct and unrelated; this is a new instance of the same *class* of mistake, in a different layer).

**Verification performed**:
- Full solution build: 0 errors. Full test suite: 124/124 passing, unchanged (no new pure-logic derivation — everything reuses already-tested resolvers, so live verification was the primary check).
- Live: `GET /api/teams/search?code=SDN` and `?q=Padres` both resolved to the same real San Diego franchise (`id: 106`).
- Live: `GET /api/teams/106/roster?season=2025` returned 51 players, including Dylan Cease — cross-checked via a direct `sqlcmd` count of distinct `PersonId`s across `Batting`/`Pitching`/`Fielding` for that franchise-season: exactly `51`, matching precisely.
- Live: `GET /api/teams/106/stats?season=2025` — every batting/fielding/pitching counting stat cross-checked via direct `sqlcmd` sums (`GameBattingStatistics`/`GameFieldingStatistics`/`Pitching`) matched exactly (plate appearances, at-bats, hits, doubles, triples, home runs, walks, strikeouts, putouts, assists, errors, games pitched, innings, earned runs) — including after catching and fixing the `Hit`/`Hits` bug above. FIP constant for SDN's league-season matched the already-established `~62.7` value from 7c (same underlying data-completeness caveat, not a new issue).
- `404` confirmed for a non-existent franchise ID on detail/roster/stats; `400` confirmed for a missing `season` on the stats route; a season with no data (1950) correctly returned `{ batting: null, pitching: null, fielding: null }` rather than erroring.

### Step 7f: Game Summary and Play-by-Play

**Status**: Complete

**What was found (starting state)**: Most of the raw data this step needs already existed and required no new repository work: `Game`/`GameModel` already carried every field a summary needs (both scores, all 5 umpire IDs, `BallparkId`, winning/losing/saving pitcher, game-winning batter, doubleheader number, attendance); `IGameLineupRepository.GetByGameIdAsync` already returned both starting lineups; `IGameBattingStatisticsRepository`/`IGamePitchingStatisticsRepository`/`IGameFieldingStatisticsRepository.GetByGameIdAsync` already returned both teams' box-score totals for one game. `GamesController` itself, however, didn't exist at all, nor did any read path for `GameSubstitution`/`GameAdjustment`/`GameComment` (built in Step 6c, write-only until now).

A genuine spec/implementation gap surfaced while researching the play-by-play requirement, found by reading `GameEventResolver.cs` and `GameContextResolver.cs` directly rather than assuming: `GameEvent`/`GameSubstitution`/`GameAdjustment`/`GameComment`'s existing `Sequence` field is **not** a shared ordinal across the four tables. Both resolvers walk the exact same `EventFileGame.Records` list -- which does preserve true chronological order -- but each keeps its own independent per-type counter (`GameEventResolver`'s `sequence` increments only on `PlayRecord`; `GameContextResolver`'s three separate counters each increment only for their own type). `api.md`'s requirement to interleave all four "using `Sequence`" assumed a shared ordinal that never actually existed in the schema -- naively unioning the four tables ordered by `Sequence` would have ordered each type correctly *within itself* while their relative interleaving was arbitrary.

**What was built**:
- Root-cause fix: a new `RecordIndex` (int) field added to `GameEvent`/`GameSubstitution`/`GameAdjustment`/`GameComment` (Contract classes + EF models), stamped by both resolvers from the position they already iterate within the shared `game.Records` list -- no new state-tracking needed, since both resolvers already loop by index over that same source list. The existing per-type `Sequence` fields are untouched. EF Core migration `AddRecordIndexForPlayByPlayInterleaving` adds the column (`NOT NULL DEFAULT 0`).
- `IGameSubstitutionRepository`/`GameSubstitutionRepository`, `IGameAdjustmentRepository`/`GameAdjustmentRepository`, `IGameCommentRepository`/`GameCommentRepository` (each just `GetByGameIdAsync`, ordered by `RecordIndex`) -- the first read paths for these three tables.
- `IGameEventRepository.GetGameEventsForDisplayAsync(gameId)`: new, separate from 7d's `GetGamesPlayByPlayAsync` (left untouched, still used by `PlayerGameLogService`) -- fetches one game's `GameEvent` rows including `GameEventFieldingCredit` (deliberately omitted by the 7d method, which doesn't need them), ordered by `RecordIndex`.
- `GameSummary`/`GameTeamBoxScore`/`GameLineupEntry` and `GamePlayByPlayEntry` Contract types (`Retrosharp.Contract.Game`/`Retrosharp.Contract.GameEvent`).
- `IGameSummaryService`/`GameSummaryService`: orchestrates `Game` + lineups + box-score totals + `Franchise`/`Ballpark`/`Person` identity resolution (umpires, decisions, lineup batters) into one `GameSummary`, using a per-request `Person` cache dictionary (same convention as 7b/7c/7e's franchise-cache pattern).
- `IGamePlayByPlayService`/`GamePlayByPlayService`: merges `GetGameEventsForDisplayAsync` + the three new context repositories by `RecordIndex` into one chronologically-ordered list, plus a glossary (`Dictionary<int, Person>`) of every distinct person referenced anywhere in the game (batters, pitchers, runners, fielding-credit players, substitution/adjustment subjects) -- resolved once per distinct person rather than embedded redundantly in every play/runner/credit.
- `IGameRepository.SearchAsync`/`IGameService.SearchAsync` (date/season/franchiseId, all optional, paginated) -- closes a gap in `api.md`'s own API Surface table: `games/search` wasn't assigned to any lettered Step 7 sub-step, but Requirement 1 mandates every listed route exist, so this step added it.
- New `GamesController`: `GET /api/games/search`, `GET /api/games/{id}`, `GET /api/games/{id}/events`. New response DTOs (`Retrosharp.UI.Api/Models/`): `GameSearchResult`, `GameSummaryResponse` (+ nested `GameTeamBoxScoreResponse`/`GameBoxScoreBatting`/`GameBoxScorePitching`/`GameBoxScoreFielding`, field names matching `GameBattingStatistics`/`GamePitchingStatistics`/`GameFieldingStatistics` exactly rather than reusing `BattingLine`/`PitchingLine`/`FieldingLine`, to avoid repeating 7e's `Hit`/`Hits` mismatch class of bug), `GamePlayByPlayResponse`/`GamePlayByPlayEntryResponse`/`GamePlayResponse`/`GameRunnerResponse`/`GameFieldingCreditResponse`/`GameSubstitutionResponse`/`GameAdjustmentResponse`/`GameCommentResponse`.
- A new unit test, `RecordIndexInterleavingTests`, proving `RecordIndex` (not each type's own unrelated `Sequence`) correctly reconstructs a mixed comment/play/adjustment/substitution/comment sequence's true file order.

**Discrepancies and decisions made during implementation**:
- Backfilling `RecordIndex` on the 162 already-imported real games required a full wipe-and-reimport of the Game Event Parser's own output (`GameEvent`/`GameEventRunner`/`GameEventFieldingCredit`/`GameSubstitution`/`GameAdjustment`/`GameComment`/`GameEventGameStatus`/`Batting`/`Pitching`/`Fielding` -- confirmed via reading `GameStatisticsRepository.TryApplyGameStatisticsAsync` that `Batting`/`Pitching`/`Fielding` are populated *exclusively* by this pipeline, idempotently). The Game Log Parser's own tables (`Game`, `GameLineup`, `GameBattingStatistics`/`GamePitchingStatistics`/`GameFieldingStatistics` -- a completely different, team-level table set) were untouched throughout. Verified as safe by confirming every previously-published verification figure (Cease's 80 H/41 R/39 ER/39 BB/123 K/275 outs, Rodríguez's 359 PA/327 AB/78 H, and all row counts) matched exactly after the reimport, byte-for-byte.
- The two real reference files (`2025SDN.EVN`/`2025SEA.EVA`) turned out to be genuinely partial-season fixtures, not full 162-game team files as earlier steps' phrasing sometimes implied -- SDN's file alone produced 81 games, SEA's alone produced the remaining 81, summing to the already-familiar `162` total. Confirmed via `Game` table cross-reference that SDN and SEA's real full schedules are each 162 games with 6 head-to-head matchups, and this project's two event files are each only about half a season.
- Reimporting both files **concurrently** (fired both `POST` requests back-to-back, as had seemed harmless for independent team files) caused a real, reproducible failure: SDN and SEA share several real interleague games, and two concurrent `BulkInsertAsync` invocations racing on the same shared `GameId` deadlocked/hit a duplicate-key error on `GameEventGameStatus`, compounded by an unrelated environment gap (the RabbitMQ vhost was missing the `nsb.v2.delay-delivery` exchange NServiceBus's delayed-retry needs, so the failed message couldn't self-heal via retry and required manual RabbitMQ queue purging and a from-scratch wipe-and-reimport). Fixed by running the two imports **sequentially** instead -- SDN to full completion, then SEA -- which completed both with zero errors, since the second file's existing-game idempotency check then correctly skipped the shared games rather than racing on them. This is a one-time backfill operational concern, not a code defect; no source change was made in response, since the existing per-game idempotency design (Step 6d) already handles sequential re-imports of overlapping data correctly, it just isn't safe to run two overlapping-schedule imports in parallel.
- The play-by-play response's `Play` field required an explicit field-by-field mapping in the controller rather than a blind `Adapt<T>()`, since `GameEventPlayRecord`'s nested `Event`/`Runners` shape doesn't structurally match `GamePlayResponse`'s flat shape -- caught live (every field read back as `0`/empty) before considering the step done, the same "verify the real response, don't assume the mapper did the right thing" discipline that caught 7e's `Hit`/`Hits` bug.
- Person identity in the play-by-play response uses a "glossary" pattern (`People: Dictionary<int, PlayerSearchResult>` keyed by `PersonId`, referenced by ID from every play/runner/credit/substitution/adjustment) rather than embedding a full player object redundantly at every occurrence -- a single game can reference the same ~20-40 people dozens of times across its play-by-play, and repeating full identity each time would bloat the response for no benefit.

**Errors encountered**:
- The `Play` field flat-vs-nested Mapster mismatch above -- caught live and fixed before reporting the step done.
- The concurrent-reimport deadlock/RabbitMQ delay-exchange issue above -- an operational hazard of this one-time backfill, not a defect in shipped code; resolved by sequencing the two imports rather than by any source change.

**Verification performed**:
- Full solution build: 0 errors. Full test suite: 125/125 passing (124 existing + 1 new `RecordIndexInterleavingTests` case).
- Reimport regression check: every counting stat already published in 7b/7c/7d/7e's progress logs for Cease/Rodríguez matched exactly after the wipe-and-reimport, plus all raw row counts (`GameEvent` 14,256, `GameEventRunner` 15,514, `GameEventFieldingCredit` 8,560, `GameSubstitution` 1,833, `GameAdjustment` 65, `GameComment` 155, `GameEventGameStatus`/`Batting`/`Pitching`/`Fielding` row counts) matched exactly, now with `RecordIndex` populated (confirmed distinct, non-default values per game).
- Live: `GET /api/games/3092` (a real SDN-vs-SEA game, 2025-05-16) returned the correct final score (1-5), box score, both real starting lineups (Tatis Jr., Machado, Merrill, Bogaerts for SDN; Rodríguez, Raleigh, Arozarena for SEA), correct decisions (winning/losing pitcher, game-winning batter), and correct umpires and ballpark (Petco Park) -- every box-score figure cross-checked exactly against direct `sqlcmd` queries of `GameBattingStatistics`/`GameFieldingStatistics`/`Game`.
- Live: `GET /api/games/3092/events` -- after fixing the flat-vs-nested mapping bug, the full 96-entry play-by-play showed correct real chronological interleaving: pitching-change substitutions and pinch-hitter substitutions appeared at exactly the right point between plays (immediately following the `NP`/"no play" placeholder Retrosheet records that precede them), not clustered by record type -- direct confirmation the `RecordIndex` fix works as intended, not just a plausible-looking unit test.
- Live: all 6 real SDN-vs-SEA head-to-head games in the imported data (`3092`, `3107`, `3122`, `4401`, `4416`, `4431`) each returned a single, complete, non-duplicated play-by-play (confirmed via `sqlcmd` row counts and a full re-check of game `4401`), demonstrating the existing per-game import idempotency (Step 6b) correctly resolves the "same physical game present in both teams' source files" case with one canonical copy, per AC5.
...
**Depends on**: Step 7 (API must exist to containerize meaningfully).

**Deliverables (revised from the original scope -- see "Discrepancies" below)**: a database engine swap from SQL Server to PostgreSQL; Dockerfiles for `Retrosharp.UI.Api`, `Retrosharp.Engine.Console`, and (new) `Retrosharp.Data.Migration`; a `docker-compose.yml` covering Postgres, RabbitMQ, and all three applications; health check endpoints; ARM64 image builds; `docs/deployment.md`.

**Definition of done**: `docker compose up` brings up a fully working stack from a clean environment, including seed data and schema migration via Step 2's mechanism, verified against real PostgreSQL rather than SQL Server LocalDB.

**What was found (starting state)**: The project's original deployment target was reconsidered before this step began -- the user intends to self-host on Raspberry Pi (ARM64) hardware, and SQL Server has no ARM64 build (Azure SQL Edge, Microsoft's one product for exactly this niche, was retired September 30, 2025). This prompted an infrastructure discussion covering database alternatives and a from-scratch Azure hobbyist-cost breakdown before any Step 8 code was written; the user chose **PostgreSQL**, deployed **fully self-hosted on the Pi** (no Azure dependency).

A subsequent research pass over the whole `src/` tree (before writing any code) found the SQL-Server surface area was narrow: 4 `UseSqlServer(...)` call sites, 3 `appsettings.json` connection strings, an `NServiceBus.Persistence.Sql` dialect selection in two `Program.cs` files (the package itself already supports a Postgres dialect), and 9 SQL-Server-specific EF Core migrations. `RetrosharpContext.OnModelCreating` had no hand-written SQL-Server-only fluent config -- the one non-obvious SQL-Server-ism (a filtered unique index on `Batting`/`Fielding`'s nullable `SeasonYear`, the actual cause of a `QUOTED_IDENTIFIER` error hit during Step 7f) was provider-auto-generated and expected to regenerate correctly under Npgsql. Both existing Dockerfiles already used Microsoft's official multi-arch base images, so no image-tag changes were needed for ARM64, only the right `--platform` flag. No `docker-compose.yml` existed yet, and no Dockerfile existed for `Retrosharp.Data.Migration` -- without one, the stated Definition of Done (migration as part of `docker compose up`) couldn't be satisfied. No health-check endpoint existed anywhere (`DiagnosticsController` is a Step-4 messaging test harness, not a health probe). Per the user's explicit decision, `Retrosharp.UI.Web` (an existing Angular frontend project, not previously mentioned anywhere in this build plan) was kept out of scope for this pass.

**What was built**:
- Database provider swap: `Microsoft.EntityFrameworkCore.SqlServer` → `Npgsql.EntityFrameworkCore.PostgreSQL` in the three affected `.csproj` files; all 4 `UseSqlServer(...)` calls → `UseNpgsql(...)`; `SqlDialect.MsSqlServer` → `SqlDialect.PostgreSql` and `Microsoft.Data.SqlClient.SqlConnection` → `Npgsql.NpgsqlConnection` in `Retrosharp.Engine.Console`/`Retrosharp.Data.Migration`'s `Program.cs`; `SqlPersistenceSchema` default changed from `"dbo"` to `"public"`; the hard-coded NServiceBus script-copy glob in `Retrosharp.Data.Migration.csproj` updated from `MsSqlServer` to `PostgreSql` (confirmed empirically -- built Engine.Console once and inspected the actual generated folder name rather than assuming); `GameStatisticsRepository.IsUniqueConstraintViolation`'s SQL-Server-specific error-code check (`SqlException { Number: 2601 or 2627 }`, the exact concurrency-safety mechanism from Step 6d) updated to Npgsql's equivalent (`PostgresException { SqlState: PostgresErrorCodes.UniqueViolation }`).
- The 9 existing SQL-Server migrations archived to `docs/archive/sqlserver-migrations/` (outside the buildable project tree, since this repo has no git history to fall back on) and replaced with one fresh `InitialCreate` generated against the Npgsql-configured context.
- `RetrosharpContext.ConfigureConventions` override mapping every `DateTime`/`DateTime?` to Postgres's `timestamp without time zone` project-wide (see "Errors encountered" -- a real bug found live, not part of the original plan).
- `IGameSubstitutionRepository`/etc.'s connection strings and the 3 `appsettings.json` files updated to Npgsql format, sourced from a new git-ignored `.env` (`.env.example` checked in) via `ConnectionStrings__DefaultConnection` environment-variable overrides -- `RetrosharpConfiguration`'s existing `AddEnvironmentVariables()` needed no code change.
- ASP.NET Core health checks added to `Retrosharp.UI.Api` (`GET /health`, real Postgres-connectivity probe via `AspNetCore.HealthChecks.NpgSql`).
- A minimal sidecar Kestrel `/health` listener added to `Retrosharp.Engine.Console` (a `FrameworkReference` to `Microsoft.AspNetCore.App` added to what stays a plain console/worker `Microsoft.NET.Sdk` project, not switched to `Sdk.Web`) -- this process has no other HTTP surface otherwise.
- New `src/lib/Retrosharp.Data.Migration/Dockerfile`, whose build context is the **repo root** (not `src/`, unlike the other two) since the project copies `docs/csv/*.csv` at build time and transitively builds `Retrosharp.Engine.Console`.
- New `docker-compose.yml` (5 services: `postgres`, `rabbitmq`, `retrosharp-migration` one-shot, `retrosharp-engine-console`, `retrosharp-ui-api`, health-based `depends_on` ordering) and `.env.example`.
- `docs/deployment.md`: setup instructions, the build-context asymmetry, ARM64 build paths (native on-Pi vs. `buildx` cross-build), known benign log lines, and a summary of the two real bugs below.
- ARM64 images built successfully for all three Dockerfiles via `docker buildx build --platform linux/arm64` (using QEMU emulation on the x64 dev machine -- no physical Pi hardware available to verify runtime behavior on).

**Discrepancies and decisions made during implementation**:
- The Deliverables/Definition of Done above are revised from what Step 8 originally specified (SQL Server, not PostgreSQL) -- a direct consequence of the infrastructure decision made before this step's implementation began, not a deviation discovered mid-implementation.
- Fixing 7e's `Hit`/`Hits`-class-of-mistake muscle memory paid off again here in a different form: rather than assuming the SQL-Server-specific `IsUniqueConstraintViolation` check would "just work" or silently fail, it was grepped for and fixed proactively during the provider swap, before it could cause a live concurrency-handling regression.
- `Retrosharp.UI.Web` (Angular frontend) confirmed out of scope for this pass per explicit user decision -- flagged as a real, previously-undocumented project the build plan doesn't cover yet, worth a decision whenever frontend work begins.

**Errors encountered** (both found live running the actual Docker Compose stack against real Postgres, not from static analysis):
1. **`DateTime` Kind mismatch**: Npgsql's EF Core provider defaults `DateTime` to Postgres's `timestamp with time zone`, which requires `Kind=Utc` and throws (`Cannot write DateTime with Kind=Unspecified to PostgreSQL type 'timestamp with time zone'`) on the `Kind=Unspecified` values this project's CSV parsers produce for birthdates/game dates/franchise eras -- plain calendar dates with no real timezone meaning, exactly what SQL Server's old `datetime2` stored without complaint. First surfaced as the `retrosharp-migration` container crashing while seeding `Franchise` rows. Fixed via `ConfigureConventions` (see above) rather than annotating ~20 affected properties individually or normalizing every parser's output; required regenerating the `InitialCreate` migration afterward (confirmed via `grep`: 16 columns now `timestamp without time zone`, 0 remaining `timestamp with time zone`).
2. **A genuine MSBuild ordering bug in the new `Retrosharp.Data.Migration` Dockerfile**: the NServiceBus script-copy `<Content Include>` glob is evaluated at MSBuild project-evaluation time, before `Retrosharp.Engine.Console` (its build-side-effect dependency) has necessarily finished building within the same `dotnet build` invocation -- on a completely fresh container this silently matched zero files (`No NServiceBus persistence scripts found... skipping`), whereas every local (non-Docker) test throughout this session had accidentally avoided the bug, since Engine.Console had always already been built moments earlier in a separate command. Fixed by adding an explicit `dotnet build` of Engine.Console as its own Dockerfile step, before building Data.Migration.
3. **`Retrosharp.Engine.Console` failed to start against a genuinely fresh RabbitMQ broker**: NServiceBus's own startup broker-verification (`BrokerVerifier.ValidateDeliveryLimit`) expects the input queue to already exist, and throws otherwise -- the existing code comment's claim that "queues form correctly without `EnableInstallers()`" had, it turned out, only ever been tested against an already-warm broker with queues already present from earlier sessions' testing (the standalone `rabbitmq` container used throughout Steps 4-7 had been running for two weeks uninterrupted). `docker-compose`'s `rabbitmq` service is genuinely queue-less on every fresh `up`, exposing this for the first time. Fixed by adding `endpointConfiguration.EnableInstallers()` -- its one known side effect (redundantly re-running `NServiceBus.Persistence.Sql`'s own schema installer) is harmless, since those scripts are idempotent.
4. **A masking bug in this step's own new health-check code, found while diagnosing #3**: the sidecar health listener's original `Task.WhenAll(host.RunAsync(), healthApp.RunAsync())` never completes when `host.RunAsync()` faults immediately, since `healthApp.RunAsync()` runs forever on its own -- the container kept reporting `healthy` (and its `RestartCount` stayed `0`) even though NServiceBus had already failed to start and the process was silently not processing any messages at all. Confirmed live: `docker top` showed the `dotnet` process still running, `docker logs` showed the startup exception, and the healthcheck still passed. Fixed with `Task.WhenAny` + awaiting (and thus rethrowing) the first-completed task, so a real startup failure now correctly exits the process and lets Docker Compose's restart policy recover it -- this is a correctness fix independent of #3, and would have masked *any* future Engine.Console startup failure, not just this one.
5. Two minor, non-functional issues fixed alongside the above: a pre-existing path bug in `Retrosharp.Engine.Console`'s own Dockerfile (missing the `engine/` prefix on its `COPY`/`WORKDIR` paths -- inconsistent with `Retrosharp.UI.Api`'s correctly-`src/`-rooted Dockerfile, and would have failed on a real multi-project build context); a benign but noisy Npgsql warning (`Cannot load library libgssapi_krb5.so.2`, probing for optional Kerberos/GSSAPI support this project doesn't use) silenced by installing `libgssapi-krb5-2` in all three Dockerfiles.

**Verification performed**:
- Full solution build: 0 errors, throughout every fix. Full test suite: 125/125 passing, unchanged (none of it touches the database provider directly).
- Live: `docker compose build` succeeded for all three images; `docker compose up -d` from a clean state (`down -v` first) brought up all 5 services, with `retrosharp-migration` exiting `0` and both `retrosharp-engine-console`/`retrosharp-ui-api` reporting Docker-healthcheck `healthy` with `RestartCount: 0` (a clean first-attempt startup, not a masked-then-self-healed one -- confirmed by re-running from scratch after the `EnableInstallers`/`WhenAny` fixes).
- Live: `retrosharp-migration`'s logs confirmed schema creation, `125 franchises added (0 already present), 656 ballparks added (0 already present)` (matching Step 2's already-established real seed-data counts exactly), and successful NServiceBus persistence schema installation, all against real PostgreSQL.
- Live: `GET /health` on the containerized UI.Api returned `Healthy`; `GET /api/teams/106` and `GET /api/teams/search?code=SDN` returned the correct real SDN Padres franchise data read from the containerized Postgres; `GET /api/players/search?q=smith` correctly returned an empty result set (no error) since biofile data hasn't been re-imported into this fresh database; `GET /api/games/999999` correctly `404`d.
...
**Messages (complete)**: new `BulkGameEventImportStart` (`BulkImportId`, `ZipPath`, `SeasonYear?`, `BatchSize?`) and `GameEventImportFailed` (`BulkImportId`, `FileName`, `Error`) in `Retrosharp.Message.GameEvent`. `Guid BulkImportId` added to `GameEventStart`, `GameEventComplete`, and `GameEventCancel` (default `Guid.Empty` = standalone single-file import). Solution builds, 231 tests still pass.

**GameEventSaga wiring (complete)**: `GameEventSagaData` carries `BulkImportId` (in the JSONB blob — not a correlation property, so no persistence schema change; `FilePath` remains the sole correlation key). `Handle(GameEventStart)` persists `message.BulkImportId` and echoes it on the `GameEventComplete` it sends. No behavior change for a standalone import (`BulkImportId` stays `Guid.Empty`). Two new `GameEventSagaTests` (`WithBulkImportId_PersistsItAndEchoesItOnComplete`, `WithoutBulkImportId_LeavesItEmptyOnComplete`); Engine.Console.Tests now 38, all 233 tests pass.

**Bulk saga (complete — not yet unit-tested or run live)**: `Retrosharp.Engine.Console/Saga/`:
- `EventFileArchive` (`internal static partial`): `IsEventFile`/`SeasonOf` (pure), `ListEventFiles`/`ExtractFiles` (zip I/O, flattened, non-event entries ignored), `TryResolveSeason` (pure — rejects empty / multi-season / implausible-year archives). Filename grammar `^\d{4}[A-Za-z]{3}\.EV[AN]$` (year-first).
- `BulkGameEventImportSagaData` + `BulkGameEventImportFileState` — correlated on `BulkImportId`; per-file state is `Pending`→`InProgress`→terminal (`Success`/`Failed`/`Skipped`), plus `BulkImportRowId`, `WorkingDirectory`, `BatchSize`, `ProcessingStarted` guard.
- `BulkGameEventImportSaga`: `Handle(BulkGameEventImportStart)` validates (readable archive → single plausible season → optional requested-season match → Game Log for that season exists via `IGameRepository.GetBySeasonAsync`), any failure → `BulkImport` row `Failed` + `FailureReason` + `MarkAsComplete`; then seeds one `BulkImportFile` per archive file (`Skipped` when its most recent prior outcome for the season is `Success`, else `Pending`), extracts only the `Pending` ones, persists via `CreateAsync` (now get-or-create on `TrackingId` for retry-safety), requests a `Watchdog` timeout, and dispatches up to `BatchSize` `GameEventStart`s (`SendLocal`, `FilePath` = working-dir path, `BulkImportId` stamped). `Handle(GameEventComplete)` / `Handle(GameEventImportFailed)` resolve a file (ignored unless it's `InProgress` — late/duplicate-safe), then dispatch the next and finish when all files are terminal. `Timeout(Watchdog)` fails every still-unfinished file and finishes. `FinishAsync` deletes only `Success` files, removes the working dir if empty, sets `Completed`/`CompletedWithFailures`, logs the summary, `MarkAsComplete`.
- `BulkGameEventImportSaga_Create.sql` (PostgreSql, `Correlation_BulkImportId uuid`) generated at build time — applied by the existing `Retrosharp.Data.Migration` `ScriptRunner.Install` mechanism, same as `GameEventSaga` in Step 6f.

Solution builds, 233 tests still pass.

**Error-queue hook (complete)**: `BulkImportFailureNotifier.OnMessageSentToErrorQueueAsync` wired into `Program.cs` via `recoverability.Failed(f => f.OnMessageSentToErrorQueue(...))`. When a message moved to the error queue is a `GameEventStart` (checked via the `NServiceBus.EnclosedMessageTypes` header) whose JSON body carries a non-empty `BulkImportId`, it `SendLocal`s a `GameEventImportFailed` (file name + `ExceptionType: message` summary, truncated) so `BulkGameEventImportSaga` can fail that file and move on; the original still sits on the error queue for a manual retry. Standalone imports (`BulkImportId == Guid.Empty`) are ignored. The endpoint's `IServiceProvider` is captured into a local set right after `host = builder.Build()` (the callback only fires at message-processing time, well after startup) and used to resolve `IMessageSession`. `EngineRecoverabilityPolicy` unchanged. Builds, 233 tests pass.

**API (complete)**: `GameEventController` gains `POST /api/gameevent/bulkimport` (validates `ZipPath` non-empty and `batchSize` positive if given, then sends `BulkGameEventImportStart` with `RequestId == BulkImportId ==` a fresh tracking id, returns `202 Accepted { trackingId }` — all deeper validation is the saga's, matching the sibling `import` endpoints) and `GET /api/gameevent/bulkimport/{trackingId:guid}` (→ `BulkImportStatusResponse` with per-status `counts` and a `files[]`; `404` when unknown). New `IBulkImportService`/`BulkImportService` (thin read wrapper over `IBulkImportRepository`, registered in `Retrosharp.Service`'s `IocRegistrations`) keeps the controller off the repository. `BulkImportStatusResponse`/`BulkImportCounts`/`BulkImportFileLine` DTOs in `Retrosharp.UI.Api/Models/` (enum statuses rendered as their names). `BulkGameEventImportStart` routed to the engine endpoint in `Retrosharp.UI.Api/Program.cs`. Builds, 233 tests pass.

**Remaining**: config (batch size, watchdog, extraction root) + the shared-volume mount in `docker-compose.yml`; `BulkGameEventImportSagaTests` + `EventFileArchive`/`BulkImportFailureNotifier` tests; live end-to-end verification.
