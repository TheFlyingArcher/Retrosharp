...
The format of the game event files is `YYYYTTT.EVN` for National League and `YYYYTTT.EVA` for American League, where `YYYY` is the year of the game, `TTT` is the three-letter team abbreviation, and `.EVN` or `.EVA` indicates the league. For example `2025SDN.EVN` is the San Diego Padres National League game event file for the year 2025.

Retrosheet's current single-season team event files contain **only that team's home games** (verified against the 2023 archive: each of the 30 files holds exactly 81 `id` records, all with the file's own team as `hometeam`). A given game therefore appears in exactly one file — its home team's — so there is no cross-file duplication to resolve, and if a team's file fails to import, that team's home games have no fallback source. The `GameEventGameStatus` atomic claim described in the [Data Model](#data-model) is retained as defence-in-depth (it costs one guarded insert per game and protects against a file that *does* carry away games — older Retrosheet layouts, a future postseason set — and against a file being reprocessed concurrently with itself), but with modern team files it never actually contends.

A whole season's team-season files are distributed by Retrosheet as a single zip archive. Importing them one API call at a time is impractical at the scale this project targets; the bulk import process that extracts such an archive and orchestrates the per-file imports covered here is specified separately in [bulk-import.md](./bulk-import.md).
